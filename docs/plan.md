...
| 9. Dedicated UI polish (feature freeze first) | 🔶 REOPENED Aug 29 as **v2 rebuild plan** (`step9-ui-polish.md`) after full UX audit — root cause found: fixed-viewBox letterboxing (blank margins + 5–9px effective SVG text), no type hierarchy, v1's 9.3 human verbs unfinished (pin + sweep-zoom agent-only, ProbeToolbar orphaned), empty cold open. Phases: 9A scale system → 9B hierarchy/hero number → 9C human parity → 9D cold open → 9E ChatGPT re-verify + rehearsal. v1 work through c27cbb2 (beam canvas, 50/50 split, pins/tray/ticker, heatmap ridges) stands. | c27cbb2 |
...
- **Step 7.5 live gate PASSED (Aug 28, Codex in-app browser)**: agent answered "safe income range" (sweep dead-zones + query_point boundary reads → "$55k–$80k, then $96k+; avoid $80k–$95k"), "when does marriage win" (diff → "always, +$21,739 at $50k"), and "how big a raise past $80k" ($95k, answered from already-fetched query_point data with zero extra calls). Bidirectional check passed: after the human clicked the $83k cliff, "what do you make of what I just did?" → agent called get_workbench, named the exact cliff, and correctly contextualized it against the bigger $80k one. `human_did_meanwhile` digest delivered human actions on first agent call.
- **Demo-video planning note**: the ChatGPT/Codex client asks a user confirmation before WebMCP calls that transmit household/financial data (its [14] sensitive-data policy — per-call context, pre-approval doesn't stick). Expect ~2 confirmation prompts in a full demo arc; script them as natural beats ("yes, use what's on the card"), don't fight them.
- **Step 8 rehearsal run 1 (Aug 28) — CRITICAL demo-script lesson**: the opening question didn't say the family was *enrolled* in childcare assistance, so the agent set receiving_childcare_subsidy=false → the NEW-APPLICANT cliff map (CCAP entry cliff −$10.6k at ~$51k, ACA cliff at ~$84k, no $80k exit cliff) → find_minimal_fix honestly answered "no whitelisted fix for aca" and the healing finale never happened. Everything the agent said was CORRECT for that household — the arc just wasn't the healing arc. Fixes: (a) set_household now replies with a check_with_the_human nudge when childcare is paid but the subsidy flag is off; (b) **the demo opening line MUST say "we're already on childcare assistance (CCAP)"** or start from the "Weighing a raise" preset (flag pre-set). Also observed: agent zoomed its sweep to $30k–$100k (so the $5k TANF cliff was out of frame — fine), assumed an income-less spouse for the marriage question but flagged the sensitivity and asked (good), and the canvas lacks a visual "you are here" marker (queued for Step 9 — the reply narrates it but the chart doesn't show it).
